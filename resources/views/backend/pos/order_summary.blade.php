	...
	<div class="col-xl-6">
		<div class="pl-xl-4">
			<div class="card mb-4">
				...
				<div class="card-body">
					@if(Session::has('pos.shipping_info') && !empty(Session::get('pos.shipping_info')['name']))
						...
						@endif
						@if(!empty(Session::get('pos.shipping_info')['phone']))
						<div class="d-flex justify-content-between mb-2">
							<span class="">{{translate('Phone')}}:</span>
							<span class="fw-600">{{ Session::get('pos.shipping_info')['phone'] }}</span>
						</div>
						@endif
						@if(!empty(Session::get('pos.shipping_info')['address']))
						<div class="d-flex justify-content-between mb-2">
						...
						@endif
						@if(!empty(Session::get('pos.shipping_info')['country']))
						<div class="d-flex justify-content-between mb-2">
							<span class="">{{translate('Country')}}:</span>
							<span class="fw-600">{{ Session::get('pos.shipping_info')['country'] }}</span>
						</div>
						@endif
						@if(!empty(Session::get('pos.shipping_info')['postal_code']))
						<div class="d-flex justify-content-between mb-2">
					...
					@else
						<div class="d-flex justify-content-between mb-2">
							<span class="">{{translate('Customer')}}:</span>
							<span class="badge badge-inline badge-info">{{ translate('Walk In Customer') }}</span>
						</div>
						<div class="text-muted small mt-2">
							{{ translate('No shipping address required.') }}
						</div>
					@endif
				</div>
			</div>
