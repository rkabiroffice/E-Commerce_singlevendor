	...
	<div>
		...
		<div style="background: #eceff4;padding: 1rem;">
			<table>
				<tr>
					<td>
						<img src="{{ pdf_image_data($logo) }}" height="60" style="display:inline-block;">
					</td>
		...
		<div style="padding: 1rem;padding-bottom: 0">
            <table>
				@php
					...
					$is_walk_in_customer = ($shipping_address->name ?? null) === translate('Walk In Customer')
						&& ($shipping_address->address ?? null) === translate('Walk In Customer')
						&& empty($shipping_address->email)
				...
				@if(!$is_walk_in_customer && (!empty($shipping_address->address) || !empty($shipping_address->city) || !empty($shipping_address->state) || !empty($shipping_address->postal_code) || !empty($shipping_address->country)))
					<tr><td class="gry-color small">{{ collect([$shipping_address->address ?? null, $shipping_address->city ?? null, isset($shipping_address->state) ? trim($shipping_address->state . (!empty($shipping_address->postal_code) ? ' - ' . $shipping_address->postal_code : '')) : ($shipping_address->postal_code ?? null), $shipping_address->country ?? null])->filter()->implode(', ') }}</td></tr>
				@endif
				@if(!$is_walk_in_customer && !empty($shipping_address->email))
					<tr><td class="gry-color small">{{ translate('Email') }}: {{ $shipping_address->email }}</td></tr>
				@endif
				@if(!$is_walk_in_customer && !empty($shipping_address->phone))
		                ...
		                @if ($orderDetail->product != null)
							<tr class="">
								<td>
                                    ...
                                    <small>
                                        @php
												$product_stock = $orderDetail->product->stocks->where('variant', $orderDetail->variation)->first() ?? $orderDetail->product->stocks->first();
                                        @endphp
	    ...
	    <div style="padding:0 1.5rem;">
	        <table class="text-right sm-padding small strong">
		        ...
		        <tbody>
			        <tr>
			            <td class="text-left">
								<strong>{{ translate('Invoice') }}:</strong>
								{{ $order->code }}
			            </td>
			            <td>
					        <table class="text-right sm-padding small strong">
