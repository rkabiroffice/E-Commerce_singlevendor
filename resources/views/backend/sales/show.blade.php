            ...
            <div class="row gutters-5">
                <div class="col text-md-left text-center">
                    @php
                        ...
                        $isWalkInCustomer = ($shippingAddress->name ?? null) === translate('Walk In Customer')
                            && ($shippingAddress->address ?? null) === translate('Walk In Customer')
                    ...
                    @endphp
                    @if($shippingAddress->name ?? false)
                        <address>
                            <strong class="text-main">
                                {{ $shippingAddress->name }}
                            </strong>
                            @if(!$isWalkInCustomer)
                                @if(!empty($shippingAddress->email))<br>{{ $shippingAddress->email }}@endif
                                @if(!empty($shippingAddress->phone))<br>{{ $shippingAddress->phone }}@endif
                                @php
                                    $addressParts = collect([
                                        $shippingAddress->address ?? null,
                                        $shippingAddress->city ?? null,
                                        isset($shippingAddress->state) ? trim($shippingAddress->state . (!empty($shippingAddress->postal_code) ? ' - ' . $shippingAddress->postal_code : '')) : ($shippingAddress->postal_code ?? null),
                                        $shippingAddress->country ?? null,
                                    ])->filter()->implode(', ');
                                @endphp
                                @if($addressParts)<br>{{ $addressParts }}@endif
                            @endif
                        </address>
                    @else
                        <address>
                            <strong class="text-main">
                                {{ optional($order->user)->name ?? translate('Walk In Customer') }}
                            </strong>
                            @if(optional($order->user)->email)<br>{{ $order->user->email }}@endif
                            @if(optional($order->user)->phone)<br>{{ $order->user->phone }}@endif
                        </address>
                    @endif
