...
@section('content')
    ...
    <div class="row">
        ...
        <div class="col-lg-6">
            <div class="card">
                <div class="card-header">
                    <h5 class="mb-0 h6">{{translate('SHADHINSMS Credential')}}</h5>
                </div>
                <div class="card-body">
                    <form class="form-horizontal" action="{{ route('update_credentials') }}" method="POST">
                        <input type="hidden" name="otp_method" value="SHADHINSMS">
                        @csrf
                        <div class="form-group row">
                            <input type="hidden" name="types[]" value="SHADHINSMS_API_URL">
                            <div class="col-lg-3">
                                <label class="col-from-label">{{translate('SHADHINSMS_API_URL')}}</label>
                            </div>
                            <div class="col-lg-6">
                                <input type="text" class="form-control" name="SHADHINSMS_API_URL" value="{{  env('SHADHINSMS_API_URL') }}" placeholder="SHADHINSMS_API_URL" required>
                            </div>
                        </div>
                        <div class="form-group mb-0 text-right">
                            <button type="submit" class="btn btn-sm btn-primary">{{translate('Save')}}</button>
