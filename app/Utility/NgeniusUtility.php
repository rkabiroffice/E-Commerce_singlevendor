...
//        $tokenResponse = self::invokeCurlRequest("POST", $idServiceURL, $tokenHeaders, null);
    ...
    public static function make_payment(string $callback_url, string $payment_type, float|int|string $amount): void
    {
        $order = new \StdClass();
    ...
    public static function check_callback(string $orderRef, string $payment_type)
    {
        $outletRef = env("NGENIUS_OUTLET_ID"); // set your outlet reference/ID value here (example only)
    ...
    public static function invokeCurlRequest(string $type, string $url, array $headers, ?string $post)
    {
        $ch = curl_init();
