...
class CheckoutController extends Controller
{
    ...
    {
        $allowedPaymentOptions = ['cash_on_delivery', 'sslcommerz'];
        if (!in_array($request->payment_option, $allowedPaymentOptions, true)) {
            flash(translate('Selected payment method is not available.'))->warning();
            return back();
        }
        ...
        if ($request->payment_option != null) {
            ...
            if ($request->session()->get('combined_order_id') != null) {
                ...
                // Only Cash on Delivery and SSLCommerz are enabled payment methods.
                $decorator = __NAMESPACE__ . '\\Payment\\' . str_replace(' ', '', ucwords(str_replace('_', ' ', $request->payment_option))) . "Controller";
                if (!class_exists($decorator)) {
                    flash(translate('Selected payment method is not configured.'))->error();
                    return back();
                }
