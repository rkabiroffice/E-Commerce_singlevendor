...
class PosController extends Controller
{
    ...
    public function set_shipping_address(Request $request) {
        if ($request->address_id != null) {
            ...
            $data['name'] = $address->user ? $address->user->name : translate('Customer');
            $data['email'] = $address->user ? $address->user->email : '';
            $data['address'] = $address->address;
            $data['country'] = $address->country ? $address->country->name : '';
            $data['state'] = $address->state ? $address->state->name : '';
            $data['city'] = $address->city ? $address->city->name : '';
            $data['postal_code'] = $address->postal_code;
            $data['phone'] = $address->phone;
        } else {
            $data['name'] = $request->name ?: translate('Walk In Customer');
            $data['email'] = $request->email ?: '';
            $data['address'] = $request->address ?: '';
            $data['country'] = $request->country_id ? (Country::find($request->country_id)->name ?? '') : '';
            $data['state'] = $request->state_id ? (State::find($request->state_id)->name ?? '') : '';
            $data['city'] = $request->city_id ? (City::find($request->city_id)->name ?? '') : '';
    ...
    public function get_order_summary(Request $request){
        ...
        return view('backend.pos.order_summary', compact('user', 'user_id'));
    }
    ...
    public function order_store(Request $request){
        if(Session::has('pos.cart') && count(Session::get('pos.cart')) > 0){
            ...
            if ($request->shipping_address != null) {
                $address = Address::find($request->shipping_address);
                if ($address) {
                    $data['name']        = $address->user ? $address->user->name : translate('Customer');
                    $data['email']       = $address->user ? $address->user->email : '';
                    $data['address']     = $address->address ?? '';
            ...
            if (empty($data) && Session::has('pos.shipping_info') && !empty(Session::get('pos.shipping_info'))) {
                $shipping_info = Session::get('pos.shipping_info');
                $data['name']        = $shipping_info['name'] ?? null;
                $data['email']       = $shipping_info['email'] ?? null;
                $data['address']     = $shipping_info['address'] ?? null;
                $data['country']     = $shipping_info['country'] ?? null;
                $data['state']       = $shipping_info['state'] ?? null;
                $data['city']        = $shipping_info['city'] ?? null;
                $data['postal_code'] = $shipping_info['postal_code'] ?? null;
                $data['phone']       = $shipping_info['phone'] ?? null;
            }

            if (empty($data)) {
                if ($request->user_id != null) {
                    $user = User::find($request->user_id);
                    $default_address = Address::where('user_id', $user->id)->where('set_default', 1)->first() ?? Address::where('user_id', $user->id)->first();
                    $data['name']        = $user ? $user->name : translate('Customer');
                    $data['email']       = $user ? $user->email : '';
                    $data['address']     = $default_address ? $default_address->address : '';
                    $data['country']     = $default_address && $default_address->country ? $default_address->country->name : '';
                    $data['state']       = $default_address && $default_address->state ? $default_address->state->name : '';
                    $data['city']        = $default_address && $default_address->city ? $default_address->city->name : '';
                    $data['postal_code'] = $default_address ? $default_address->postal_code : '';
                    $data['phone']       = $default_address ? $default_address->phone : ($user->phone ?? '');
                } else {
                    $data['name']        = translate('Walk In Customer');
                    $data['email']       = '';
                    $data['address']     = translate('Walk In Customer');
                    $data['country']     = '';
                    $data['state']       = '';
                    $data['city']        = '';
                    $data['postal_code'] = '';
                ...
                if (empty($data['name'])) {
                    $data['name'] = $request->user_id ? (User::find($request->user_id)->name ?? translate('Customer')) : translate('Walk In Customer');
                }
                if (empty($data['address'])) {
                    $data['address'] = $request->user_id ? '' : translate('Walk In Customer');
                }
            }
            ...
            if($request->payment_type == 'offline_payment'){
                ...
                $offline_data['name']   = $request->offline_payment_method;
                $offline_data['amount'] = $request->offline_payment_amount;
                $offline_data['trx_id'] = $request->offline_trx_id;
                $offline_data['photo']  = $request->offline_payment_proof;
                $order->manual_payment_data = json_encode($offline_data);
            ...
            if($order->save()){
                ...
                if(env('MAIL_USERNAME') != null){
                    try {
                        if (!empty($data['email'])) {
                            Mail::to($data['email'])->queue(new InvoiceEmailManager($array));
                        }
                        $adminUser = User::where('user_type', 'admin')->first();
                        if ($adminUser && !empty($adminUser->email)) {
                            Mail::to($adminUser->email)->queue(new InvoiceEmailManager($array));
                        }
                    } catch (\Exception $e) {
